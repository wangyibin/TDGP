...
def alleleTableFromGmap(args):
    ...
    all_gene_df['all_gene'] = all_gene_df.parallel_apply(
        filter_by_blast, axis=1, args=(blast_df,))
    all_gene_df['all_gene'] = all_gene_df.all_gene.apply(
        lambda d: d if d else np.nan)
    ...
    allele_df = pd.DataFrame(
        columns=args.gene_headers + ['dup_gene'], index=index_genes)
    ...
    res_df.dup_gene = res_df.dup_gene.apply(
    ...
    final_rmdup_all_df = rmdup_all_df.apply(
        formatAlleleTable, 1, args=(args.gene_headers, ))
