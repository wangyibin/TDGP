...
%(prog)s <sample_1000000_iced.cool> [Options]
    ...
    plotting boxplot of cis interaction per chromosome.
"""
...
def main(args):
    ...
    output = args.matrix.rsplit('.', 1)[0] + '_CisInteraction_boxplot.pdf' \
    ...
    for i, chroms in enumerate(chrom_homo_pairs):
        ...
        for chrom in chroms:
            ax = axes_list[i]
            chr_matrix = getChromPairsMatrix(cool, hm, chrom, chrom)
            chr_matrix.sort(axis=1)
            chr_data.append(method_func(chr_matrix, axis=1))
        bplot = ax.boxplot(chr_data,
                showfliers=False, 
                patch_artist=True, 
                notch=True, 
                widths=0.35,
